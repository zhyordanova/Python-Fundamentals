...
def heal(heroes_dict, hero_name, amount):
    current_hit_points = heroes_dict[hero_name]["HP"]
    if current_hit_points + amount >= 100:
        heroes_dict[hero_name]["HP"] = 100
        print(f"{hero_name} healed for {100 - current_hit_points} HP!")
    else:
...
for _ in range(n):
    ...
    heroes[name] = {"HP": hit_point, "MP": mana_points}
